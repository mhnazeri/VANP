...
def create_samples(
    input_path, obs_window: int = 6, pred_window: int = 8, linear_threshold: int = 0.03
) -> dict:
    ...
    for i in range(traj_len - seq_len):
        ...
        positions.append(data["position"][i : i + obs_window].astype(np.float32))
        goal_positions.append(
            data["position"][i + obs_window : i + seq_len].astype(np.float32)
        )
        # past and future yaw
        yaws.append(data["yaw"][i : i + obs_window].astype(np.float32))
        goal_yaws.append(data["yaw"][i + obs_window : i + seq_len].astype(np.float32))
        # past and future vw
        vws.append(data["vw"][i : i + obs_window].astype(np.float32))
        goal_vws.append(data["vw"][i + obs_window : i + seq_len].astype(np.float32))
        # store image addresses
        past_frames.append(all_frames[i : i + obs_window])
        goal_frames.append(all_frames[i + obs_window : i + seq_len])
        non_linears.append(
            poly_fit(
                data["position"][i + obs_window : i + seq_len],
                pred_window,
                linear_threshold,
            )
        )
        non_linear_counter += poly_fit(
            data["position"][i + obs_window : i + seq_len],
            pred_window,
            linear_threshold,
        )
...
if __name__ == "__main__":
    ...
    if args.create_samples:
        ...
        print(
            f"Created {len(base_dict['past_frames'])} samples from data in {save_path.parent} directory!"
        )
